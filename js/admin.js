...
async function publishToGitHub() {
  ...
  try {
    ...
    if (getRes.ok) {
      ...
      sha = cur.sha;
      // decode remote data.json เพื่อ preserve fields ที่ admin ไม่ได้แก้
      try {
        remoteData = JSON.parse(atob(cur.content.replace(/\n/g, '')));
      } catch(_) {}
    } else if (getRes.status !== 404) {
      throw new Error('GET ' + getRes.status + ': ' + (await getRes.text()).slice(0,200));
    }

    // merge: เอา remote เป็น base แล้ว overlay ด้วย DATA ของ admin
    // เพื่อ preserve fields ที่แก้ผ่าน git เช่น includedSections, ritualNote ในแต่ละ package
    let merged = DATA;
    if (remoteData) {
      merged = Object.assign({}, remoteData, DATA);
      // merge packages แบบ deep: preserve per-package fields ที่ไม่มีใน admin UI
      const PRESERVED_PKG_FIELDS = ['includedSections', 'ritualNote'];
      for (const cat of Object.keys(DATA.packages || {})) {
        if (!remoteData.packages?.[cat]) continue;
        merged.packages[cat] = DATA.packages[cat].map(adminPkg => {
          const remotePkg = remoteData.packages[cat].find(r => r.id === adminPkg.id);
          if (!remotePkg) return adminPkg;
          const out = Object.assign({}, adminPkg);
          for (const field of PRESERVED_PKG_FIELDS) {
            if (remotePkg[field] !== undefined && adminPkg[field] === undefined) {
              out[field] = remotePkg[field];
            }
          }
          return out;
        });
